...
def main():
    ...
    # add semaphore here
    # update in the optimization loop or in the object post step
    @ray.remote
    class oxDNARunFlag:
        def __init__(self):
            self.ok_to_run = False

        def check_bin_ready(self) -> bool:
            return self.ok_to_run

        def set_bin_status(self, status:bool) -> None:
            self.ok_to_run = status

    run_flag = oxDNARunFlag.remote()
    ...
    def make_simulator(id:str, disable_build:bool) -> jdna_simulator.BaseSimulator:
        ...
        simulator = oxdna.oxDNASimulator(
            ...
            check_build_ready=lambda: ray.get(run_flag.check_bin_ready.remote()),
            set_build_ready=run_flag.set_bin_status.remote,
        )
