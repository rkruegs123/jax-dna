...
for no_outliers in [False, True]:
# for no_outliers in [True]:
# for no_outliers in [False]:
    ...
    for width, height in [(20, 14), (24, 14), (28, 14)]:
        ...
        outlier_iters = onp.array([52, 53])
        xs = onp.arange(len(fin_gs))
        if no_outliers:
            min_outlier_iter = onp.min(outlier_iters)
            max_outlier_iter = onp.max(outlier_iters)

            ax.scatter(fin_ref_iters[onp.where(fin_ref_iters < min_outlier_iter)[0]], fin_ref_gs[onp.where(fin_ref_iters < min_outlier_iter)[0]], color="black", s=100)
            ax.scatter(fin_ref_iters[onp.where(fin_ref_iters > max_outlier_iter)[0]], fin_ref_gs[onp.where(fin_ref_iters > max_outlier_iter)[0]], color="black", s=100)

            ax.plot(xs[onp.where(xs < min_outlier_iter)[0]], fin_gs[onp.where(xs < min_outlier_iter)[0]], color="black", linewidth=3)
            ax.plot(xs[onp.where(xs > max_outlier_iter)[0]], fin_gs[onp.where(xs > max_outlier_iter)[0]], color="black", linewidth=3)

            dashed_xs = onp.array([min_outlier_iter - 1, max_outlier_iter + 1])
            ax.plot(dashed_xs, fin_gs[dashed_xs], color="black", linewidth=2, linestyle="--")

        else:
            ax.plot(xs, fin_gs, color="black", linewidth=3)
