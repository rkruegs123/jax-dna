...
ptwist_dir = data_dir / "ptwist-opt/"
...
for width, height in [(20, 14)] :
    ...
    fin_pitches_path = ptwist_dir / "obj/means_i150.npy"
    fin_pitches = onp.load(fin_pitches_path)

    fin_ref_iters_path = ptwist_dir / "obj/ref_iters_i150.npy"
    fin_ref_iters = onp.load(fin_ref_iters_path)

    fin_ref_pitches_path = ptwist_dir / "obj/ref_means_i150.npy"
    fin_ref_pitches = onp.load(fin_ref_pitches_path)

    ax.plot(fin_pitches, color="black", linewidth=6)
    ax.scatter(fin_ref_iters, fin_ref_pitches, color="black", s=300)
    ax.axhline(y=target_ptwist, linestyle="--", color="red", label="Target Prop. Twist", linewidth=5)

    ax.set_xlabel("Iteration")
    ax.set_ylabel("Propeller Twist (deg)")

    y_vals = [19, 20, 21]
    ax.set_yticks(y_vals)
    ax.set_yticklabels([r'$19$', r'$20$', r'$21$'])

    x_vals = [0, 75, 150]
    ax.set_xticks(x_vals)
    ax.set_xticklabels([r'$0$', r'$75$', r'$150$'])

    plt.tight_layout()

    # plt.show()
    plt.savefig(output_dir / f"ptwists_losses.pdf")
    plt.close()


# Load the gradients from the first iteration
grads_path = ptwist_dir / "log"/ "grads_iter0.txt"
...
for width, height in [(20, 14), (15, 8), (20, 8)]:
    ...
    # plt.show()
    plt.savefig(output_dir / f"ptwist_top{m}_grads_{width}x{height}.pdf")
    plt.close()
