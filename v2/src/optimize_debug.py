...
def run(args, init_params,
    ...
    skip_propeller = args['skip_propeller']
    skip_helical = args['skip_helical']
    skip_pitch_quartets = args['skip_pitch_quartets']
    ...
    if save_output:
        ...
        run_name = f"optimize_debug_{timestamp}_b{batch_size}_lr{lr}_n{sim_length}"
    ...
    if loss_type == "structural":
        ## Construct sets of pairs for structural loss function
        backbone_dist_pairs = top_info.bonded_nbrs
        helical_pairs = top_info.bonded_nbrs[skip_helical:-skip_helical]
    ...
    for i in tqdm.trange(opt_steps, position=0):
        ...
        if i % save_every == 0 and save_output:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--loss-type', type=str,
                        default="structural",
                        help='Tye type of loss function',
                        choices=["structural"])
    parser.add_argument('--save-output', action='store_true')

    ## Structural loss debug (Note: maybe can also use for mechanical)
    ...
    parser.add_argument('--skip-propeller', type=int, default=1,
                        help="Number of base pairs to skip for propeller twist")
    parser.add_argument('--skip-pitch-quartets', type=int, default=1,
                        help="Number of quartets to skip for pitch")
    parser.add_argument('--skip-helical', type=int, default=1,
                        help="Number of base pairs to skip for helical radius")
    ...
    start = time.time()
    run(args, init_params=init_params, save_output=save_output)
    end = time.time()
