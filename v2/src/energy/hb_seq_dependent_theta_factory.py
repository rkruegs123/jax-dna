...
from utils import HB_WEIGHTS, get_hb_probs, stacking_param_names, GC_CG_WEIGHTS, AT_TA_WEIGHTS
...
def energy_fn_factory(displacement_fn,
    ...
    fene_fn = Partial(v_fene, **params["fene"])
    ...
    ## Construct partial hydrogen bonding function
    hb_params = params["hydrogen_bonding"]
    fixed_hb_param_keys = [
        "dr_low_hb", "dr_high_hb", "dr_c_low_hb", "dr_c_high_hb", "eps_hb", "a_hb",
        "dr0_hb", "dr_c_hb", "b_low_hb", "b_high_hb",
        "delta_theta_star_hb_1", "delta_theta_hb_1_c", "b_hb_1",
        "delta_theta_star_hb_2", "delta_theta_hb_2_c", "b_hb_2",
        "delta_theta_star_hb_3", "delta_theta_hb_3_c", "b_hb_3",
        "delta_theta_star_hb_4", "delta_theta_hb_4_c", "b_hb_4",
        "delta_theta_star_hb_7", "delta_theta_hb_7_c", "b_hb_7",
        "delta_theta_star_hb_8", "delta_theta_hb_8_c", "b_hb_8"
    ]
    fixed_hb_params = {k: hb_params[k] for k in fixed_hb_param_keys}
    hb_fn_partial = partial(hydrogen_bonding, **fixed_hb_params)
    ...
    def _compute_subterms(body: RigidBody, seq: util.Array, params, op_nbrs_idx):
        ...
        # Compute the GC and AT hydrogen bonding functions
        a_params = params[0]
        theta_params = params[1]
        gc_params = a_params[0, :] # params is 2x6
        at_params = a_params[1, :]
        hb_fn_gc = partial(hb_fn_partial,
                           a_hb_1=gc_params[0], a_hb_2=gc_params[1], a_hb_3=gc_params[2],
                           a_hb_4=gc_params[3], a_hb_7=gc_params[4], a_hb_8=gc_params[5],
                           theta0_hb_1=theta_params[0][0], theta0_hb_2=theta_params[0][1],
                           theta0_hb_3=theta_params[0][2], theta0_hb_4=theta_params[0][3],
                           theta0_hb_7=theta_params[0][4], theta0_hb_8=theta_params[0][5] 
        )
        hb_fn_at = partial(hb_fn_partial,
                           a_hb_1=at_params[0], a_hb_2=at_params[1], a_hb_3=at_params[2],
                           a_hb_4=at_params[3], a_hb_7=at_params[4], a_hb_8=at_params[5],
                           theta0_hb_1=theta_params[1][0], theta0_hb_2=theta_params[1][1],
                           theta0_hb_3=theta_params[1][2], theta0_hb_4=theta_params[1][3],
                           theta0_hb_7=theta_params[1][4], theta0_hb_8=theta_params[1][5] 
        )
        ...
        # Note: computation for v_hb is redundant, because we really don't have to be calling it for every base pair
        hb_probs = get_hb_probs(seq, op_i, op_j) # get the probabilities of all possibile hydrogen bonds for all neighbors

        v_hb_gc = hb_fn_gc(dr_base_op, theta1_op, theta2_op, theta3_op, theta4_op,
                           theta7_op, theta8_op)
        v_hb_gc = jnp.where(mask, v_hb_gc, 0.0) # Mask for neighbors
        hb_weights_gc = jnp.dot(hb_probs, GC_CG_WEIGHTS)
        hb_dg_gc = jnp.dot(hb_weights_gc, v_hb_gc) # dot product to only be between appropriate bases

        v_hb_at = hb_fn_at(dr_base_op, theta1_op, theta2_op, theta3_op, theta4_op,
                           theta7_op, theta8_op)
        v_hb_at = jnp.where(mask, v_hb_at, 0.0) # Mask for neighbors
        hb_weights_at = jnp.dot(hb_probs, AT_TA_WEIGHTS)
        hb_dg_at = jnp.dot(hb_weights_at, v_hb_at) # dot product to only be between appropriate bases

        hb_dg = hb_dg_gc + hb_dg_at
