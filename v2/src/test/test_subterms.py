...
def run(top_path, traj_path, input_path, T=DEFAULT_TEMP, use_neighbors=False):
    print(f"----Checking energy breakdown agreement for trajectory at location: {traj_path}----")

    computed_subterms = compute_subterms(top_path, traj_path, T, use_neighbors=use_neighbors)
