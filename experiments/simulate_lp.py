...
def sim_scan(conf_info, top_info, n_steps, key, save_every):
    init_body = conf_info.get_states()[0]
    seq_oh = jnp.array(utils.get_one_hot(top_info.seq), dtype=jnp.float64)
...
if __name__ == "__main__":
    ...
    if method == "for-loop":
        traj = sim_for_loop(conf_info, top_info, n_steps, key, save_every=sample_every)
    elif method == "scan":
        traj = sim_scan(conf_info, top_info, n_steps, key, save_every=sample_every)
    ...
    quartets = utils.get_all_quartets(n_nucs_per_strand=traj[0].center.shape[0] // 2)
