...
def animate_directory(basedir, show_plot, save_plot, save_basedir):
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 8), width_ratios=[2, 1])
    bars = list()
    ...
    for res_idx in range(num_res):
        ...
        tmp_bar = ax1.bar(x_labels, res_weights, bottom=bottom0, label=DNA_ALPHA[res_idx], color=colors[res_idx])
        bars.append(tmp_bar)
        bottom0 += res_weights

    ax1.set_title("Residue Distribution")
    ax1.set_xlabel("Residue")
    ax1.set_ylabel("Probability")
    # ax.legend(loc="center right")

    ax2.set_title("Rg Evolution")
    ax2.set_xlabel("Iteration")
    ax2.set_ylabel("Rg (oxDNA units)")


    ax2.set_ylim(rgs.min() * 0.9, rgs.max() * 1.1)
    ...
    def update(frame):
        ...
        # ax2_xs.append(frame)
        # ax2_ys.append(rgs[frame])
        ax2_xs = onp.arange(num_frames)[:frame]
        ax2_ys = rgs[:frame]
        line2.set_data(ax2_xs, ax2_ys)

        return [ret_val, line2]

    ani = animation.FuncAnimation(fig, update, frames=num_frames, interval=3, blit=False)

    handles, labels = ax1.get_legend_handles_labels()
    # ax.legend(handles[::-1], labels[::-1], title='Residue', loc="center right")
    ax1.legend(handles[::-1], labels[::-1], title='Residue', bbox_to_anchor=(1.0, 0.85))
