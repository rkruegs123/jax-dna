...
def run(args):
    ...
    def get_ref_states(params, i, seed, prev_basedir):
        ...
        for rs_idx in range(n_traj_states):
            ...
            ref_pitches.append(avg_pitch)
        ref_pitches = onp.array(ref_pitches)


        running_avg = onp.cumsum(ref_pitches) / onp.arange(1, n_traj_states + 1)
        plt.plot(running_avg)
        plt.savefig(iter_dir / f"running_avg.png")
        plt.clf()

        plt.plot(running_avg[-int(n_traj_states // 2):])
        ...
        return traj_states, calc_energies, jnp.array(ref_pitches), iter_dir

    # Construct the loss function
    @jit
    def loss_fn(params, ref_states, ref_energies, ref_pitches):
        ...
        expected_pitch = jnp.dot(weights, ref_pitches)
        ...
        return rmse, (n_eff, expected_pitch)
    ...
    ref_states, ref_energies, ref_pitches, ref_iter_dir = get_ref_states(params, i=0, seed=0, prev_basedir=prev_ref_basedir)
