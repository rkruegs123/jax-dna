...
from jax import jit, vmap, lax, value_and_grad, tree_util, flatten_util
...
def run(args):
    ...
    init_params_dict = deepcopy(model.EMPTY_BASE_PARAMS)
    for opt_key in opt_keys:
        if seq_avg:
            init_params_dict[opt_key] = deepcopy(model.default_base_params_seq_avg[opt_key])
        else:
            init_params_dict[opt_key] = deepcopy(model.default_base_params_seq_dep[opt_key])
    ...
    def get_ref_states(params_flat, i, seed, prev_states_force, prev_states_torque, prev_basedir, resample_hpin, last_hpin_iter):
        params = ravel_fn(params_flat)
    ...
    def pitch_loss_fn(params_flat, bp60_ref_info):
        params = ravel_fn(params_flat)
        ...
        return loss_pitch, (expected_pitch, n_eff_60bp)
    pitch_grad_fn = value_and_grad(pitch_loss_fn, has_aux=True)
    pitch_grad_fn = jit(pitch_grad_fn)

    def lp_loss_fn(params_flat, bp60_ref_info):
        params = ravel_fn(params_flat)
        ...
        return loss_lp, (expected_lp, expected_rise, expected_lp_n_bp, expected_l0_avg, n_eff_60bp)
    lp_grad_fn = value_and_grad(lp_loss_fn, has_aux=True)
    lp_grad_fn = jit(lp_grad_fn)


    def seff_loss_fn(params_flat, stretch_tors_ref_info):
        params = ravel_fn(params_flat)
        ...
        return loss_s_eff, (s_eff, n_effs_f, n_effs_t, c, g, a1, a3, a4)
    seff_grad_fn = value_and_grad(seff_loss_fn, has_aux=True)
    seff_grad_fn = jit(seff_grad_fn)

    def c_loss_fn(params_flat, stretch_tors_ref_info):
        params = ravel_fn(params_flat)
        ...
        return loss_c, (c, n_effs_f, n_effs_t, s_eff, g, a1, a3, a4)
    c_grad_fn = value_and_grad(c_loss_fn, has_aux=True)
    c_grad_fn = jit(c_grad_fn)

    def g_loss_fn(params_flat, stretch_tors_ref_info):
        params = ravel_fn(params_flat)
        ...
        return loss_g, (g, n_effs_f, n_effs_t, s_eff, c, a1, a3, a4)
    g_grad_fn = value_and_grad(g_loss_fn, has_aux=True)
    g_grad_fn = jit(g_grad_fn)

    def hpin_loss_fn(params_flat, hpin_ref_info):
        params = ravel_fn(params_flat)
        ...
        return loss_hpin, (curr_tm, n_eff_hpin, curr_width)
    hpin_grad_fn = value_and_grad(hpin_loss_fn, has_aux=True)
    hpin_grad_fn = jit(hpin_grad_fn)
    ...
    last_hpin_iter = None
    stretch_tors_ref_info, prev_last_states_force, prev_last_states_torque, bp60_ref_info, hpin_ref_info, ref_iter_dir, did_compute_hpin = get_ref_states(params_flat, i=0, seed=30362, prev_states_force=None, prev_states_torque=None, prev_basedir=prev_ref_basedir, resample_hpin=True, last_hpin_iter=last_hpin_iter)
    if did_compute_hpin:
        last_hpin_iter = 0
    ...
    for i in tqdm(range(n_iters)):
        ...
        n_eff_60bp = None
        if use_pitch:
            (loss_pitch, (expected_pitch, n_eff_60bp)), pitch_grads = pitch_grad_fn(params_flat, bp60_ref_info)
            all_grads.append(pitch_grads)
            loss += loss_pitch
        else:
            loss_pitch = 0.0
            expected_pitch = -1
        ...
        if use_lp:
            (loss_lp, (curr_lp, expected_rise, curr_lp_n_bp, curr_l0_avg, n_eff_60bp)), lp_grads = lp_grad_fn(params_flat, bp60_ref_info)
            all_grads.append(lp_grads)
            loss += loss_lp
        else:
            loss_lp = 0.0
            curr_lp = -1
        ...
        n_effs_f = None
        n_effs_t = None
        a1, a3, a4 = None, None, None
        if use_s_eff:
            (loss_s_eff, (s_eff, n_effs_f, n_effs_t, c, g, a1, a3, a4)), s_eff_grads = seff_grad_fn(params_flat, stretch_tors_ref_info)
            all_grads.append(s_eff_grads)
            loss += loss_s_eff
        else:
            loss_s_eff = 0.0
        if use_c:
            (loss_c, (c, n_effs_f, n_effs_t, s_eff, g, a1, a3, a4)), c_grads = c_grad_fn(params_flat, stretch_tors_ref_info)
            all_grads.append(c_grads)
            loss += loss_c
        else:
            loss_c = 0.0
        if use_g:
            (loss_g, (g, n_effs_f, n_effs_t, s_eff, c, a1, a3, a4)), g_grads = g_grad_fn(params_flat, stretch_tors_ref_info)
            all_grads.append(g_grads)
            loss += loss_g
        else:
            loss_g = 0.0
        if n_effs_f is None:
            n_effs_f = jnp.full((n_forces,), n_sample_states_st*n_sims_st)
        if n_effs_t is None:
            n_effs_t = jnp.full((n_torques,), n_sample_states_st*n_sims_st)
        if s_eff is None:
            s_eff = -1
        if c is None:
            c = -1
        ...
        if use_hpin:
            (loss_hpin, (expected_tm, n_eff_hpin, expected_width)), hpin_grads = hpin_grad_fn(params_flat, hpin_ref_info)
            all_grads.append(hpin_grads)
            loss += loss_hpin
        else:
            loss_hpin = 0.0
            expected_tm = -1
        ...
        if resample or num_resample_iters >= max_approx_iters:
            ...
            start = time.time()
            stretch_tors_ref_info, prev_last_states_force, prev_last_states_torque, bp60_ref_info, new_hpin_ref_info, ref_iter_dir, did_compute_hpin = get_ref_states(params_flat, i=i, seed=i, prev_states_force=prev_last_states_force, prev_states_torque=prev_last_states_torque, prev_basedir=prev_ref_basedir, resample_hpin=resample_hpin, last_hpin_iter=last_hpin_iter)
            end = time.time()
            if did_compute_hpin:
                last_hpin_iter = i
            if resample_hpin:
        ...
        grads_str = f"\nIteration {i}:"
        for k, v in ravel_fn(grads).items():
            grads_str += f"\n- {k}"
            for vk, vv in v.items():
                grads_str += f"\n\t- {vk}: {vv}"
...
def get_parser():
    ...
    parser.add_argument('--use-s-eff', action='store_true')
    parser.add_argument('--use-c', action='store_true')
    parser.add_argument('--use-g', action='store_true')
    ...
    parser.add_argument('--use-hpin', action='store_true')
