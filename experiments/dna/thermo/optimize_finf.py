...
def run(args):
    ...
    def get_ref_states(params, i, seed, prev_basedir):
        ...
        ## Unbias reference counts, logging running average of finfs
        ...
        for rs_idx in tqdm(range(n_ref_states)):
            ...
            if rs_idx >= running_avg_min and rs_idx % running_avg_freq == 0:
                curr_finf = tm.compute_finf(ref_unbiased_counts)
        ...
        ## Compute final finfs
        calc_finf = tm.compute_finf(ref_unbiased_counts)
        ref_finf = tm.compute_finf(last_hist_df['count_unbiased'].to_numpy())
    ...
    def loss_fn(params, ref_states, ref_energies, all_ops, all_op_weights):
        ...
        def unbias_scan_fn(unb_counts, rs_idx):
            ...
            weighted_add_term = n_ref_states/difftre_weight * 1/op_weight
