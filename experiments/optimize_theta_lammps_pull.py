...
def run(args):
    ...
    def get_ref_states(params, i, seed):
        ...
        plt.plot(all_thetas)
        plt.savefig(iter_dir / "theta_traj.png")
        plt.clf()
    ...
    @jit
    def loss_fn(params, ref_states, ref_energies, ref_thetas):
        ...
        mse = (expected_theta - target_theta)**2
        rmse = jnp.sqrt(mse)

        return rmse, (n_eff, expected_theta)
    ...
    start = time.time()
    ref_states, ref_energies, ref_thetas = get_ref_states(params, i=0, seed=30362)
    end = time.time()
    with open(resample_log_path, "a") as f:
    ...
    for i in tqdm(range(n_iters)):
        ...
        (loss, (n_eff, curr_theta)), grads = grad_fn(params, ref_states, ref_energies, ref_thetas)
        num_resample_iters += 1

        if i == 0:
            all_ref_losses.append(loss)
            all_ref_times.append(i)
            all_ref_thetas.append(curr_theta)
        ...
        if n_eff < min_n_eff or num_resample_iters >= max_approx_iters:
            ...
            start = time.time()
            ref_states, ref_energies, ref_thetas = get_ref_states(params, i=i, seed=i)
            end = time.time()
            with open(resample_log_path, "a") as f:
        ...
        with open(neff_path, "a") as f:
            f.write(f"{n_eff}\n")
        with open(theta_path, "a") as f:
            f.write(f"{curr_theta}\n")
        ...
        plt.plot(onp.arange(i+1), all_thetas, linestyle="--", color="blue")
        plt.scatter(all_ref_times, all_ref_thetas, marker='o', label="Resample points", color="blue")
        plt.axhline(y=target_theta, linestyle='--', label="Target Theta", color='red')
        plt.legend()
        plt.ylabel("Expected Theta (rads)")
        plt.xlabel("Iteration")
        plt.savefig(img_dir / f"thetas_iter{i}.png")
        plt.clf()
