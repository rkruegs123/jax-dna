...
def run(args):
    ...
    def get_ref_states(params, i, seed):
        ...
        all_gt_energies = utils.tree_stack(all_gt_energies)
        ...
        for r in range(n_sims):
            repeat_thetas = all_thetas[r]
            force_avgs = onp.mean(repeat_thetas, axis=0)
            thetas_per_kb = force_avgs * (1000 / 36)
            # theta_diffs = thetas_per_kb - thetas_per_kb[0]
        ...
        for r in range(n_sims):
            ...
            plt.scatter(forces_pn, theta_diffs, label=f"r{r}, slope={slope_offset0}")
        plt.legend()
