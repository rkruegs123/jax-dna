...
def run(args):
    ...
    for force in tqdm(forces, desc="Generating trajectories"):
        ...
        with open(log_path, "a") as f:
            f.write(f"- force {force}: {end - start} seconds\n")

        f_trajs[force] = f_traj
    ...
    for force in forces:
        ftraj = f_trajs[force]
        pdists[force] = vmap(compute_dist)(ftraj)
    end = time.time()
    with open(log_path, "a") as f:
    ...
    for i in range(num_running_avg_points):
        f_lens = list()
        for f in forces:
            f_lens.append(all_running_averages[f][i])
    ...
    for force in forces:
        ftraj = f_trajs[force]
        start = time.time()
        ftraj_info = trajectory.TrajectoryInfo(
            top_info, read_from_states=True, states=ftraj, box_size=conf_info.box_size)
        ftraj_info.write(run_dir / f"traj_f{force}.dat", reverse=True)
        end = time.time()
        with open(log_path, "a") as f:
            f.write(f"- force {force}: {end - start} seconds\n")
