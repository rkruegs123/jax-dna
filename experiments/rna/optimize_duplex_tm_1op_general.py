...
def run(args):
    ...
    # t_kelvin = args['temp']
    init_t_kelvin = args['init_sim_temp']
    ...
    unique_topology = args['unique_topology']
    ...
    sample_t_kelvin_path = log_dir / "sample_t_kelvin.txt"
    ...
    def get_ref_states(params, i, seed, prev_basedir, t_kelvin):
        ...
        energy_fn = lambda body: em_base.energy_fn(
            ...
            unique_hb_pairs=unique_hb_pairs
        )
        ...
        for extrap_t_kelvin, extrap_kt in zip(extrapolate_temps, extrapolate_kts):
            ...
            temp_unbiased_counts = onp.zeros(n_bp+1)
            ...
            fig, ax = plt.subplots(1, 2, figsize=(12, 5))
            sns.barplot(x=jnp.arange(n_bp+1), y=temp_unbiased_counts, ax=ax[0])
    ...
    def loss_fn(params, ref_states, ref_energies, all_ops, all_op_weights, sample_t_kelvin):
        kT = utils.get_kt(sample_t_kelvin)
        beta = 1 / kT

        em = model.EnergyModel(displacement_fn, params, t_kelvin=sample_t_kelvin, salt_conc=salt_concentration)
    ...
    sample_t_kelvin = init_t_kelvin
    ref_states, ref_energies, ref_ops, ref_op_weights, ref_iter_dir = get_ref_states(params, i=0, seed=key, prev_basedir=prev_ref_basedir, t_kelvin=sample_t_kelvin)
    prev_ref_basedir = deepcopy(ref_iter_dir)
    end = time.time()
    with open(resample_log_path, "a") as f:
        f.write(f"Finished generating initial reference states. Took {end - start} seconds.\n\n")
    with open(sample_t_kelvin_path, "a") as f:
    ...
    for i in tqdm(range(n_iters)):
        ...
        (loss, (curr_tm, curr_width, n_eff)), grads = grad_fn(params, ref_states, ref_energies, ref_ops, ref_op_weights, sample_t_kelvin)
        ...
        if n_eff < min_n_eff or num_resample_iters > max_approx_iters:
            ...
            delta_init_tm = curr_tm - init_tm
            sample_t_kelvin = init_t_kelvin + delta_init_tm
            with open(sample_t_kelvin_path, "a") as f:
                f.write(f"{sample_t_kelvin}\n")

            start = time.time()
            ref_states, ref_energies, ref_ops, ref_op_weights, ref_iter_dir = get_ref_states(params, i=i, seed=key+1+i, prev_basedir=prev_ref_basedir, t_kelvin=sample_t_kelvin)
            end = time.time()
            prev_ref_basedir = deepcopy(ref_iter_dir)
            with open(resample_log_path, "a") as f:
                f.write(f"- time to resample: {end - start} seconds\n\n")

            (loss, (curr_tm, curr_width, n_eff)), grads = grad_fn(params, ref_states, ref_energies, ref_ops, ref_op_weights, sample_t_kelvin=sample_t_kelvin)
...
def get_parser():
    ...
    parser.add_argument('--oxdna-path', type=str,
                        ...
                        help='Run name')
    parser.add_argument('--init-sim-temp', type=float, default=324.15,
                        help="Initial simulation temperature in Kelvin")
    ...
    parser.add_argument('--unique-topology', action='store_true')
