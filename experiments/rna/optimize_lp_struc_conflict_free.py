...
def run(args):
    ...
    # Initialize parameters and ravelling
    ...
    init_params_dict = {"seq_avg": seq_avg_params}
    init_params_flat, ravel_fn = flatten_util.ravel_pytree(init_params_dict)


    # Do the simulation
    def get_ref_states(params_flat, i, seed, prev_basedir):
    ...
    def rise_loss_fn(
        ...
        return rmse_rise, (n_eff_struc, expected_rise)
    rise_grad_fn = value_and_grad(rise_loss_fn, has_aux=True)
    rise_grad_fn = jit(rise_grad_fn)
    ...
    def pitch_loss_fn(
        ...
        return rmse_pitch, (n_eff_struc, expected_pitch, expected_angle)
    ...
    for i in tqdm(range(n_iters)):
        ...
        (rmse_lp, (n_eff_lp, curr_lp, expected_corr_curve, curr_rise_lp, curr_offset)), lp_grads = lp_grad_fn(params_flat, ref_states, ref_energies, unweighted_corr_curves, unweighted_rises)

        (rmse_rise, (n_eff_struc_rise, curr_rise)), rise_grads = rise_grad_fn(params_flat, ref_states_struc, ref_energies_struc, unweighted_rises_struc)
        ...
        curr_lp_n_bp = curr_lp / curr_rise
        ...
        if i % plot_every == 0 and i:
            ...
            plt.plot(onp.arange(i+1), all_rises, linestyle="--", color="blue")
            plt.scatter(all_ref_times, all_ref_rises, marker='o', label="Resample points", color="blue")
            plt.axhline(y=target_rise, linestyle='--', label="Target Rise", color='red')
            plt.legend()
            plt.ylabel("Expected Rise (nm)")
            plt.xlabel("Iteration")
            plt.savefig(img_dir / f"rises_iter{i}.png")
            plt.clf()
            ...
            plt.legend()
            plt.ylabel("Expected Pitch (# bp / turn)")
            plt.xlabel("Iteration")
...
def get_parser():
    ...
    parser.add_argument('--oxdna-path', type=str,
                        default="/home/ryan/Documents/harvard/research/oxdna-bin/bin1/oxDNA/",
