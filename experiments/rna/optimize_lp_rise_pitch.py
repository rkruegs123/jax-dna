...
def run(args):
    ...
    def loss_fn(
        ...
        ref_states_struc, ref_energies_struc, unweighted_rises, unweighted_angles
    ):
        em = model.EnergyModel(
            displacement_fn,
            params["seq_avg"],
        ...
        rmse_total = rmse_lp + rmse_rise + rmse_pitch

        return rmse_total, (n_eff_lp, expected_lp, expected_corr_curve, expected_rise_lp, expected_offset, n_eff_struc, expected_rise, expected_pitch, expected_lp_n_bp, rmse_lp, rmse_rise, rmse_pitch)
    ...
    ref_states, ref_energies, unweighted_corr_curves, unweighted_rises, ref_iter_dir, ref_states_struc, unweighted_rises_struc, unweighted_angles_struc, ref_energies_struc = get_ref_states(params, i=0, seed=0, prev_basedir=prev_ref_basedir)
    ...
    for i in tqdm(range(n_iters)):
        ...
        (loss, (n_eff_lp, curr_lp, expected_corr_curve, curr_rise_lp, curr_offset, n_eff_struc, curr_rise, curr_pitch, curr_lp_n_bp, rmse_lp, rmse_rise, rmse_pitch)), grads = grad_fn(params, ref_states, ref_energies, unweighted_corr_curves, unweighted_rises, ref_states_struc, ref_energies_struc, unweighted_rises_struc, unweighted_angles_struc)
        ...
        if n_eff_lp < min_n_eff_lp or n_eff_struc < min_n_eff_struc or num_resample_iters >= max_approx_iters:
            ...
            start = time.time()
            ref_states, ref_energies, unweighted_corr_curves, unweighted_rises, ref_iter_dir, ref_states_struc, unweighted_rises_struc, unweighted_angles_struc, ref_energies_struc = get_ref_states(params, i=i, seed=i, prev_basedir=prev_ref_basedir)
            end = time.time()
            prev_ref_basedir = deepcopy(ref_iter_dir)
            with open(resample_log_path, "a") as f:
