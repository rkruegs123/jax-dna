...
def GenRanSeqGU(length,keys):
	...
	for i in range(length):
		...
		if( i > 0):
			last_bp = sek[i-1] + compsek[length - 1 - (i-1)]
			current_bp = base + inverted_base
			#print last_bp, pair,
			while (last_bp == 'GU' or last_bp == 'UG' ) and ( current_bp == 'GU' or current_bp == 'UG'  ):
				#print 'rejected , trying ',
				pair = random.choice(keys) 
				base = pair[0]
				inverted_base = pair[1]
				current_bp = base + inverted_base
...
print >> sys.stderr,filled['GUGU'],filled['UGUG'], filled['GUUG'], filled['UGGU']
