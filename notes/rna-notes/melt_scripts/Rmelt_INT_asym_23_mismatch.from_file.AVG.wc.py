...
def GetAvgTm(seqlength,looplengthX,looplengthY,stack_dh,stack_ds,miss_dh,miss_ds,box):
#takes sequence in 5'-3' directioni, loop start is the index of the first element of the sequence that i in the loop
	looplength = looplengthX + looplengthY
	...
	length = seqlength -2 #seqlenthg is the number of bp outside the loops
	...
	termMis_dH = np.mean(miss_dh.values())
	termMis_dS = np.mean(miss_ds.values())
	DH += termMis_dH
	DS += termMis_dS
...
if len(sys.argv) < 5:
	print 'Usage: ./program sequence_length  no_of_nucs_in_the_missX no_of_nucs_in_the_missY box'
	sys.exit(1)


sequence = int(sys.argv[1])

mislength  = int(sys.argv[2])


mislengthB = int(sys.argv[3])
...
RealTemp = GetAvgTm(sequence,looplengthX,looplengthY, newdh_stack, newds_stack,newdh_mis,newds_mis,box)
