...
#unpaired ends
#this would be for mean
'''
avgDH_3 = np.mean(np.array(unpaired3_dHs))
avgDS_3 = np.mean(np.array(unpaired3_dSs))
avgDH_5 = np.mean(np.array(unpaired5_dHs))
avgDS_5 = np.mean(np.array(unpaired5_dSs))
'''

if len(sys.argv) < 3:
	print 'Usage: ./program LENGTH box [3/5] overhang_index(0..3)'
	sys.exit(1)
...
if overhang_type == '3':
	avgDH_3 = unpaired3_dHs[overhang_index] 
	avgDS_3 = unpaired3_dSs[overhang_index] 
	DH += 2*avgDH_3
	DS += 2*avgDS_3
	print >> sys.stderr, ' 3 overhang',avgDH_3, avgDS_3 

elif overhang_type == '5':
	avgDH_5 = unpaired5_dHs[overhang_index] 
	avgDS_5 = unpaired5_dSs[overhang_index] 
	DH += 2*avgDH_5
	DS += 2*avgDS_5
