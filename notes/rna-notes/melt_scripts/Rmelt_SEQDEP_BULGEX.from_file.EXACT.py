...
#takes sequence in 5'-3' directioni, loop start is the index of the first element of the sequence that i in the loop
	...
	if looplength == 1:
		...
		pairA = sequence[loop_start-1] + compseq[-loop_start] 
		if pairA in ['GU','UG','AU','UA']:
			DS += 1000.0 * 0.4/(273.15 + 37)		
		pairB = sequence[loop_start+1] + compseq[-loop_start-1]
		if pairB in ['GU','UG','AU','UA']:
			DS += 1000.0 * 0.4/(273.15 + 37)		
