...
def GenHpinSeq(stem,loop):
	...
	while not is_correct_loop:
		loopA, nic = GenRanSeq(loop)
		if not are_complementary(loopA[0],loopA[-1]):
			is_correct_loop = 1	
...
if len(sys.argv) != 4:
	print 'Usage: ./program howmany stemLENGTH loopLENGTH'
	sys.exit(1)
