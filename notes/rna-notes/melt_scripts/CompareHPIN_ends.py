...
def get_avg_term_mis_HS(ranindex):
	DH_vals = []
	DS_vals = []
	...
	if ranindex <  len(DH_vals):
		return DH_vals[ranindex], DS_vals[ranindex]
	else:
		return 0,0
...
def Get_TM_for_some_end(stemlength,looplength,end_index):
	...
	termMis_dH_avg, termMis_dS_avg = get_avg_term_mis_HS(end_index) 

	if termMis_dH_avg == 0 and termMis_dS_avg == 0:
 		return 0
...
end_index = 0
while ok:
	Tm = Get_TM_for_some_end(stemlength,looplength,end_index)
	end_index += 1
	if Tm > 0:
		allTMs.append(Tm)
	else:
