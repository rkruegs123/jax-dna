...
def GetWidth(deltaH,deltaS,molarconcentration):
	molarconcentration /= 2.
	Tmin = deltaH / (deltaS - GAS_CONSTANT * math.log(20.0/molarconcentration ))
	Tmax = deltaH / (deltaS - GAS_CONSTANT * math.log(5.0/(molarconcentration*16.0 )) )
	
	print 'dH: %f, dS: %f ' % (deltaH,deltaS) 
	print Tmin
	print Tmax
...
def GetW(seq,compseq,DH_vals,DS_vals):
	...
	Width = GetWidth(DH*1000.,DS,molcon)
...
if len(sys.argv) < 3:
	print 'Usage: ./program SEQUENCE COMPSEQUENCE(orWorO) box'
	sys.exit(1)
...
RealTemp = GetW(sequence,compsequence,newdh,newds)
