...
def are_wc_wobble_complementary(baseA,baseB):
...
for base_a in bases:
        for base_b in bases:
                if are_complementary(base_a, base_b):
                        for overTa in bases:
				for overTb in bases:
					if not are_wc_wobble_complementary(overTa,overTb):
						newsequence = sequence[:stemlength-1] + base_a + overTa + sequence[stemlength+1:stemlength+looplength-1] + overTb + base_b + sequence[stemlength+looplength+1:] 	
						RealTemp = GetTM(newsequence[::-1],stemlength,looplength, newdh_stack, newds_stack, newdh_t, newds_t)

						print newsequence ,'G', RealTemp 
