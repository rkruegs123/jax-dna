...
def GetWidth(deltaH,deltaS):
	Tmin = deltaH / (deltaS - k_GAS_CONSTANT*math.log(4.)) 
	Tmax = deltaH / (deltaS - k_GAS_CONSTANT * math.log(1./4 ))
...
def show_yield(deltaH, deltaS,Tmin,Tmax,Td):
	T = Tmin
	while T < Tmax:
		T += Td
		phi = math.exp( (-1.0/(k_GAS_CONSTANT*T)) * (deltaH - T * deltaS   ) )
		fraction = phi / (1.0 + phi)
		print T, fraction
...
width = GetWidth(DH*1000,DS)

s = GenHpinSeq(stemlength+1,looplength)
print s , RealTemp , width

show_yield(DH*1000,DS,RealTemp - width/2.0 - 5, RealTemp + width/2. + 5, 3.)
