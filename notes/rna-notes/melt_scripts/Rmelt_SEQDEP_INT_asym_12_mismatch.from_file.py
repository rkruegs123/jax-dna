...
hstackDG = DATAFILEPATH + 'int21.dat'
hstackDH = DATAFILEPATH + 'int21.dh'
...
def ReadParams(filename):
	...
	while i < (len(lines)):
		line = lines[i]
		if len(line.split() ) == 6 and line.split() == ['Y','Y','Y','Y','Y','Y']:
			inttypesA = lines[i+6]
			inttypesB = lines[i+7]	
			extra_nuc_type = lines[i+8].split()[0][1]

			itypesAtemp = inttypesA.split()
			itypesBtemp = inttypesB.split()
			itypesA = []
			itypesB = []
			for k in range(len(itypesAtemp)/2):
				#itypesA.append(itypesAtemp[2*k] + itypesAtemp[2*k + 1])
				#itypesB.append(itypesBtemp[2*k] + itypesBtemp[2*k + 1])
				itypesA.append(itypesAtemp[2*k] + itypesBtemp[2*k])
				itypesB.append(itypesAtemp[2*k+1] + itypesBtemp[2*k + 1])

			vals = {}
			vals[0] = lines[i+10].split()
			vals[1] = lines[i+11].split()
			vals[2] = lines[i+12].split()
			vals[3] = lines[i+13].split()
		

			for j in range(len(itypesA)):	
					basestrtype = itypesA[j] + itypesB[j]
					for x in range(4):
						for y in range(4):
							strtype = basestrtype + num_to_base[x] + num_to_base[y] + extra_nuc_type 
							#if(strtype == 'GCGCAGC'):
							#	print 'Filling the key for basestr',i,j,basestrtype,' and ',x,num_to_base[x] , extra_nuc_type,  y,num_to_base[y],vals[x] 
							energy = vals[x][j*4+y]
							if energy != '.' and not are_complementary(num_to_base[x],num_to_base[y]): #and not are_complementary(num_to_base[x],extra_nuc_type)
...
def GetRealTM(sequence,compseq,loop_start,looplengthX,looplengthY,stack_dh,stack_ds,miss_dh,miss_ds,box=20.):
#takes sequence in 5'-3' directioni, loop start is the index of the first element of the sequence that i in the loop
	...
	if( not (looplengthX == 1 and looplengthY == 2) ):
		print 'Incorrect loop lengths'
		return -10
	...
	i = loop_start - 1
	termmis_key = sequence[i] + compseq[-i-1] 
	key += termmis_key	
	print 'Miskey is ',termmis_key	
	...
	key += sequence[loop_start] + compseq[-loop_start-1] + compseq[-loop_start-2]
	termMis_dH = miss_dh[key] 
	termMis_dS = miss_ds[key]
	DH += termMis_dH
	DS += termMis_dS
	print 'The KEY is',key, ' with dH = ', miss_dh[key]
