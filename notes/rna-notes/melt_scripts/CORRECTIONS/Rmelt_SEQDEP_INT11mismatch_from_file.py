...
hstackDG = DATAFILEPATH + 'int11.dat'
hstackDH = DATAFILEPATH + 'int11.dh'
...
def Read_INT_11_Params(filename):
	fin = open(filename,'r')
	lines = fin.readlines()
	i = 0
	final_en = {}
...
def Read_INT_11_DSandDH(fileDH,fileDG):
	dgT = 37 + 273.15

	
	finalDG = Read_INT_11_Params(fileDG)
	finalDH = Read_INT_11_Params(fileDH)
	finalDS = {}
	for key in finalDH.keys():
		finalDS[key] = 1000.0 * (finalDH[key] - finalDG[key]) / dgT
...
#takes sequence in 5'-3' directioni, loop start is the index of the first element of the sequence that i in the loop
	...
	#mismatches 
	i = loop_start - 1
	termmis_key = sequence[i] + sequence[i+2] + compseq[-1-i] + compseq[-i-1-2] +  sequence[i+1] + compseq[-i-1-1]  
	print 'Mismatch 11 key',termmis_key
	#print miss_dh
...
newds_mis, newdh_mis = Read_INT_11_DSandDH(hstackDH,hstackDG)
