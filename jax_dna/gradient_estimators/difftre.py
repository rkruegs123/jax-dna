...
def build_energy_function(
    ...
    initialized_energy_fns = [
        ...
        for op, e_fn, e_c in zip(opt_params, energy_fns, energy_configs)
    ]
...
def compute_states_energies(
    ...
    n_eq_steps: int,
    sample_every: int,
    energy_fn: Callable[[dict[str, float]], Callable],  # build_energy_function
    ...
    #  calculate
    ref_states = trajectory.rigid_body[n_eq_steps::sample_every]
    ref_energies = energy_fn(params)(ref_states)
...
def compute_loss(
    opt_params: list[dict[str, float]],
    energy_fn: Callable[[dict[str, float]], Callable],
    beta: float,
    ...
    new_energies = energy_fn(opt_params)(ref_states)
    ...
    return reduce_fn(jnp.array(losses)), (n_eff, losses)


def compute_gradients(
    opt_params: list[dict[str, float]],
    key: jax.random.PRNGKey,
    loss_w_grads: Callable,
    min_n_eff: int,
    build_cl: Callable,
) -> tuple[list[dict[str, float]], tuple[float, ...], tuple[float, ...]]:
    (loss, (n_eff, losses)), grads = loss_w_grads(opt_params)

    # if n_eff is greater than the threshold we don't need to recompute the
    # reference states and energies.
    if n_eff >= min_n_eff:
        return grads, loss, n_eff, losses
    else:
        key, split = jax.random.split(key)
        loss_w_grads = build_cl(opt_params, split)

        (loss, (n_eff, losses)), grads = loss_w_grads(opt_params)

    return grads, loss, losses, loss_w_grads
...
#         (loss, (n_eff, losses)), grads = jax.jit(jax.value_and_grad(self.compute_loss, has_aux=True))(
#             opt_params,
#             loss_fns,
#             self.trajectory,
#             self.ref_states,
