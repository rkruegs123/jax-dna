...
from jax_md import rigid_body, simulate, space
from tqdm import tqdm

from jax_dna.common import topology, trajectory, utils
from jax_dna.common.interactions import (
    coaxial_stacking,
    cross_stacking,
    exc_vol_bonded,
    exc_vol_unbonded,
...
from jax_dna.common.utils import DEFAULT_TEMP, Q_to_back_base, Q_to_base_normal, Q_to_cross_prod, clamp
from jax_dna.dna1.load_params import _process, load
...
def add_coupling(base_params):
    ...
    base_params["cross_stacking"]["delta_theta_star_cross_3"] = base_params["cross_stacking"][
...
class EnergyModel:
    ...
    def compute_subterms(self, body, unpaired_pseq, bp_pseq, bonded_nbrs, unbonded_nbrs):
        ...
        exc_vol_bonded_dg = exc_vol_bonded(
            dr_base_nn, dr_back_base_nn, dr_base_back_nn, **self.params["excluded_volume_bonded"]
        ).sum()
        ...
        def compute_seq_dep_weight(nt1, nt2, weights_table, flattened_weights_table):
            nt1_unpaired = self.is_unpaired[nt1]
            nt2_unpaired = self.is_unpaired[nt2]

            # Case 1: Both unpaired
            pair_probs = jnp.kron(
                unpaired_pseq[self.idx_to_unpaired_idx[nt1]], unpaired_pseq[self.idx_to_unpaired_idx[nt2]]
            )
            pair_weight_unpaired = jnp.dot(pair_probs, flattened_weights_table)

            # Case 2: nt1 unpaired, nt2 base paired
            nt1_nt_probs = unpaired_pseq[self.idx_to_unpaired_idx[nt1]]
            nt2_bp_idx, within_nt2_bp_idx = self.idx_to_bp_idx[nt2]
            bp_probs = bp_pseq[nt2_bp_idx]

            def nt1_up_fn(nt1_nt, nt2_bp_type_idx):
                nt2_nt = BP_IDXS[nt2_bp_type_idx][within_nt2_bp_idx]
            ...
            pair_weight_nt1_up = vmap(vmap(nt1_up_fn, (None, 0)), (0, None))(
                jnp.arange(N_NT), jnp.arange(N_BP_TYPES)
            ).sum()

            # Case 3: nt2 unpaired, nt1 base paired

            nt2_nt_probs = unpaired_pseq[self.idx_to_unpaired_idx[nt2]]
            nt1_bp_idx, within_nt1_bp_idx = self.idx_to_bp_idx[nt1]
            bp_probs = bp_pseq[nt1_bp_idx]

            def nt2_up_fn(nt2_nt, nt1_bp_type_idx):
                nt1_nt = BP_IDXS[nt1_bp_type_idx][within_nt1_bp_idx]
            ...
            def same_bp_fn(bp_idx):
                ...
                bp_nt1, bp_nt2 = BP_IDXS[bp_idx][jnp.array([within_nt1_bp_idx, within_nt2_bp_idx])]
            ...
            return jnp.where(
                ...
                jnp.where(
                    nt1_unpaired,
                    pair_weight_nt1_up,
                    jnp.where(nt2_unpaired, pair_weight_nt2_up, pair_weight_both_paired),
                ),
            )

        stack_weights = vmap(compute_seq_dep_weight, (0, 0, None, None))(
            nn_i, nn_j, self.ss_stack_weights, self.ss_stack_weights_flat
        )
        ...
        exc_vol_unbonded_dg = exc_vol_unbonded(
            dr_base_op, dr_backbone_op, dr_back_base_op, dr_base_back_op, **self.params["excluded_volume"]
        )
        ...
        v_hb = hydrogen_bonding(
            ...
            theta7_op,
            theta8_op,
            **self.params["hydrogen_bonding"],
        )

        v_hb = jnp.where(mask, v_hb, 0.0)  # Mask for neighbors
        hb_weights = vmap(compute_seq_dep_weight, (0, 0, None, None))(
            op_i, op_j, self.ss_hb_weights, self.ss_hb_weights_flat
        )
        hb_dg = jnp.dot(hb_weights, v_hb)

        cr_stack_dg = cross_stacking(
            r_base_op, theta1_op, theta2_op, theta3_op, theta4_op, theta7_op, theta8_op, **self.params["cross_stacking"]
        )
        ...
        return fene_dg, exc_vol_bonded_dg, stack_dg, exc_vol_unbonded_dg, hb_dg, cr_stack_dg, cx_stack_dg
...
class TestDna1(unittest.TestCase):
    ...
    def test_brute_force_new(self):
        ...
        model_base = orig_model.EnergyModel(
            displacement_fn, t_kelvin=t_kelvin, ss_hb_weights=ss_hb_weights, ss_stack_weights=ss_stack_weights
        )
        ...
        for struc_idx in range(n_eval_strucs):
            ...
            expected_energy_calc = energy_fn(state, unpaired_pseq, bp_pseq, top_info.bonded_nbrs, neighbors_idx)
            ...
            for seq_idxs in tqdm(all_seq_idxs, desc="Seq idxs."):
                ...
                seq_energy_calc = energy_fn_base(state, seq_oh, top_info.bonded_nbrs, neighbors_idx)
