...
from jax_dna.cgdna.dsdna import dsDNA
...
class cgDNA:
    def __init__(self, sequence, ps='ps1'):
        if ps == 'dna_ps1':
            ...
            raise RuntimeError(f"Invalid parameter set: {ps}")
    ...
    def plot2D(self, *cgDNA, **kwargs):
        ...
        for key, value in kwargs.items():
            if value in Input:
                ...
                msg = f"Unkown input keyword: {value}."
                msg += " Possible keywords are: RotDeg (default = 0), LineWidth (defautl = 1)"
        ...
        if RotDeg == 1:
            ...
            raise RuntimeError(f"Invalid value for RotDeg: {RotDeg}")
    ...
    def MonteCarlo(self, NbrSamples, Drop_base=0):
        ...
        try:
            LT = scipy.sparse.csc_matrix(np.linalg.cholesky(self.stiff.todense()).T)
        except:
            LT = scipy.sparse.csc_matrix(np.linalg.cholesky(self.stiff).T)

        ttc = np.zeros(self.nbp-1 - 2*Drop_base)
        s = np.add(scipy.sparse.linalg.spsolve(LT, conf),
                   mb.repmat(self.ground_state, 1, 1).T)
